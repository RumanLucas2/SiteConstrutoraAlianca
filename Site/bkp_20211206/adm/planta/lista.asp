...
<%
	...
	if Ordena = "" Then
		Ordena = "id_galeria DESC"	'Variável Ordena recebe valor "Nome"
	...
	sql =	"SELECT * FROM tbl_galeria_planta " & Nome_SQL & " ORDER BY " & Ordena & ""
	'Executa
...
<div id="global">
    ...
    <div id="content">
        ...
        <h2><span></span>Galeria de Plantas</h2>
        <div class="content_divisao"></div>      
        
        <a href="frm_add.asp?lista=todos&amp;limpa=ok" class="content_link1">Nova Galeria</a> | 
        ...
        <form name="selecao" id="selecao" action="act_exc.asp" method="POST">
       	...
       	<table class="content_table">
            <tr>
                ...
                <th><div class="content_table_title_arrow"></div><a href="lista.asp?Ordena=nome"><strong>Galeriad e Planta</strong></a></th>
        ...
        <%
			Else
				...
				Empreendimento = Rs("id_empreendimento")
				
					sqlEmpreendimento = "SELECT id_empreendimento, nome FROM tbl_empreendimento WHERE id_empreendimento = " & Empreendimento
					Set RsEmpreendimento = conexao.execute(sqlEmpreendimento)
					
					if Not RsEmpreendimento.EOF then
						nome_empreendimento = RsEmpreendimento("nome")
					end if
		%>
        
        <tr class="content_table_line1">
        	
            <td onClick="location.href='frm_vis.asp?id_galeria=<%=Rs("id_galeria")%>'"><%=nome_empreendimento%></td>
            
            <td><%=Rs("nome")%></td>
            
           	<td>
          		<% if Rs("status") = "1" then %>
               		<a href="change_on.asp?id=<%= Rs("id_galeria") %>"><span class="icon on"></span></a>
                <% else %>
                	<a href="change_off.asp?id=<%= Rs("id_galeria") %>"><span class="icon off"></span></a>
                <% end if %>
			</td>
           
            <td>
            	<a href="lista_fot.asp?id_galeria=<%=Rs("id_galeria")%>" class="icon foto"><span class="displace">Fotos</span></a>
                <a href="frm_edt.asp?edit=ok&id_galeria=<%= Rs("id_galeria") %>" class="icon edit"><span class="displace">Editar</span></a>
                <a href="frm_vis.asp?id_galeria=<%=Rs("id_galeria")%>" class="icon visualize"><span class="displace">Visualizar</span></a>
            </td>
         	
            <td align="right">
            		<input type="Checkbox" name="excluirid" value="<%= Rs("id_galeria") %>" class="styled" />
            </td>
