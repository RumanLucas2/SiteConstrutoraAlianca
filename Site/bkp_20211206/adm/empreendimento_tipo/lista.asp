...
<%	
	...
	sql =	"SELECT * FROM tbl_empreendimento_tipo " & Nome_SQL & " ORDER BY " & Ordena & ""
	'Executa
...
<div id="global">
    ...
    <div id="content">
        ...
        <h2><span></span>Tipos de Empreendimentos</h2>
        <div class="content_divisao"></div>      
        
        <a href="frm_add.asp?lista=todos&amp;limpa=ok" class="content_link1">Novo Tipo</a> | 
        ...
        <tr class="content_table_line1">
            ...
            <td onClick="location.href='frm_vis.asp?id=<%=Rs("id_tipo")%>'"><%=nome%></td> 
           	<td>
          		<% if Rs("status") = 1 then %>
               		<a href="change_on.asp?id=<%= Rs("id_tipo") %>"><span class="icon on"></span></a>
                <% else %>
                	<a href="change_off.asp?id=<%= Rs("id_tipo") %>"><span class="icon off"></span></a>
                <% end if %>
			</td>
           
            <td>
                <a href="frm_edt.asp?edit=ok&id=<%= Rs("id_tipo") %>" class="icon edit"><span class="displace">Editar</span></a>
                <a href="frm_vis.asp?id=<%=Rs("id_tipo")%>" class="icon visualize"><span class="displace">Visualizar</span></a>
            </td>
         	
            <td align="right">
            	<input type="Checkbox" name="excluirid" value="<%= Rs("id_tipo") %>" class="styled" />
                <!-- class="styled" -->
            </td>
