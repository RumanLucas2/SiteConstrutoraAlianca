...
<%	
	...
	sql =	"SELECT * FROM tbl_empreendimento " & Nome_SQL & " ORDER BY " & Ordena & ""
	'Executa
...
<div id="global">
    ...
    <div id="content">
        ...
        <h2><span></span>Empreendimentos</h2>
        <div class="content_divisao"></div>      
        
        <a href="frm_add.asp?lista=todos&amp;limpa=ok" class="content_link1">Novo Empreendimento</a> | 
        ...
        <form name="selecao" id="selecao" action="act_exc.asp" method="POST">
       	...
       	<table class="content_table">
            <tr>
                ...
                <th><div class="content_table_title_arrow"></div><a href="lista.asp?Ordena=tipo"><strong>Tipo</strong></a></th>
                ...
                <th colspan="3"><div class="content_table_title_arrow"></div><a href="lista.asp?Ordena=status"><strong>Destaque</strong></a></th>
            </tr>
        ...
        <tr class="content_table_line1">
            ...
            <td onClick="location.href='frm_vis.asp?id=<%=Rs("id_empreendimento")%>'"><%=nome%></td>
            <td>
				
                <%
					sqlTipo = "SELECT nome FROM tbl_empreendimento_tipo WHERE id_tipo = " & tipo
            ...
            <td>
           	...
           	<td>
          		<% if Rs("status") = 1 then %>
               		<a href="change_on.asp?id=<%= Rs("id_empreendimento") %>"><span class="icon on"></span></a>
                <% else %>
                	<a href="change_off.asp?id=<%= Rs("id_empreendimento") %>"><span class="icon off"></span></a>
                <% end if %>
			</td>
            
            <td>
          		<% if Rs("destaque") = 1 then %>
               		<a href="destaque_on.asp?id=<%= Rs("id_empreendimento") %>"><span class="icon on"></span></a>
                <% else %>
                	<a href="destaque_off.asp?id=<%= Rs("id_empreendimento") %>"><span class="icon off"></span></a>
                <% end if %>
			</td>
           
            <td>
                <a href="frm_andamento.asp?id=<%= Rs("id_empreendimento") %>" class="icon texto"><span class="displace">Andamento</span></a>
                <a href="frm_edt.asp?edit=ok&id=<%= Rs("id_empreendimento") %>" class="icon edit"><span class="displace">Editar</span></a>
                <a href="frm_vis.asp?id=<%=Rs("id_empreendimento")%>" class="icon visualize"><span class="displace">Visualizar</span></a>
            </td>
         	
            <td align="right">
            	<input type="Checkbox" name="excluirid" value="<%= Rs("id_empreendimento") %>" class="styled" />
                <!-- class="styled" -->
            </td>
