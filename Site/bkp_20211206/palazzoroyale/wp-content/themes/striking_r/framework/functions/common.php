...
function theme_get_logo(){
	$logo = json_decode(wpml_t(THEME_NAME, 'Logo Image Source',  json_encode(
		theme_get_option('general','logo') 
	)));
	if(is_object($logo)){
		return (array)$logo;
	} else {
...
function theme_get_mobile_logo(){
	$logo = json_decode(wpml_t(THEME_NAME, 'Logo Image Source For Mobile Devices',  json_encode(
		theme_get_option('general','mobile_logo') 
	)));
	if(is_object($logo)){
		return (array)$logo;
	} else {
