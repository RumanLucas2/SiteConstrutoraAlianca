		...
		while ($r->have_posts()){
			...
			if($image_source){
				...
				if($lightbox == 'true'){
					...
					$output .= '<div class="masonry_item_image"><a href="'.$href.'"'.$link_target.' title="'.sprintf( __("Permanent Link to %s", 'theme_front'), get_the_title() ).'"><img src="'.$image_src.'" alt="'.get_the_title().'" /><span class="masonry_item_image_overlay"></span></a></div>';
				}
			}
			if($title){
				$output .= '<h4 class="masonry_item_title"><a href="'.$href.'"'.$link_target.' rel="bookmark" title="'.sprintf( __("Permanent Link to %s", 'theme_front'), get_the_title() ).'">'.get_the_title().'</a></h4>';
			}
			if($desc){
